...
describe('data', () => {
    ...
    context('PiBoardInfo', () => {
        ...
        it('Works with Pi B', ()=> {
            const r = (new PiBoardInfo()).from_revision(0x900011);
        ...
        it('Raises PinUnknownPi', () => {
            expect(() => {
                /*eslint no-new: off*/
                (new PiBoardInfo()).from_revision(0x9000f1)
            }).to.throw(PinUnknownPi);

        });
        it('Has physical pins data', () => {
            //Assert physical pins for some well-known Pi's; a21041 is a Pi2B
            assert.shallowDeepEqual ((new PiBoardInfo()).from_revision(0xa21041).physical_pins('3V3') , [{header: 'P1', pin: 1}, {header: 'P1', pin: 17}]);
            assert.shallowDeepEqual ((new PiBoardInfo()).from_revision(0xa21041).physical_pins('GPIO2') , [{header: 'P1', pin: 3}]);
            assert.shallowDeepEqual ((new PiBoardInfo()).from_revision(0xa21041).physical_pins('GPIO47') , []);
        });

        it('Has physical pin data', () => {
            //Assert physical pins for some well-known Pi's; a21041 is a Pi2B
            expect(() => {
                (new PiBoardInfo()).from_revision(0xa21041).physical_pin('GND');
            }).to.throw(PinMultiplePins);

            assert.shallowDeepEqual ((new PiBoardInfo()).from_revision(0xa21041).physical_pin('GPIO3') , {header: 'P1', pin: 5});

            expect(() => {
               (new PiBoardInfo()).from_revision(0xa21041).physical_pin('GPIO47');
            }).to.throw(PinNoPins);
        });

        it('Indicates if a pin is pulled up', () =>{
            assert((new PiBoardInfo()).from_revision(0xa21041).pulled_up('GPIO2') === true , "GPIO2 should be pulled up");

            assert((new PiBoardInfo()).from_revision(0xa21041).pulled_up('GPIO4') === false , "GPIO4 should not be pulled up");

            expect(() => {
                (new PiBoardInfo()).from_revision(0xa21041).pulled_up('GPIO47');
            }).to.throw(PinNoPins);
        });
